...
router.route('/:id')
  //get an algorithm
  .get((req, res, next) => {
      mongoDriver.getOne('recipes', {_id: req.params.id}, (err, result) => {
          if (err) {
  ...
  .put((req, res, next) => {
      Validator.validateUpdate(req.body, (err, response) => {
          if (err) {
              ...
              mongoDriver.updateOne('recipes', req.params.id, response, (err, result, updated) => {
                  if (err) {
                      err.status = code.HTTP_INTERNAL_SERVER_ERROR;
                      return next(err);
                  } else if (!updated) {
  ...
  //remove an algorithm
  .delete((req, res, next) => {
      mongoDriver.deleteOne('recipes', req.params.id, (err, result) => {
          if (err) {
              err.status = code.HTTP_INTERNAL_SERVER_ERROR;
              return next(err);
          } else if (!result) {
              const err = new Error('recipe not deleted');
              err.status = code.HTTP_BAD_REQUEST;
              return next(err);
          } else {
              res.sendStatus(code.HTTP_NO_CONTENT);
